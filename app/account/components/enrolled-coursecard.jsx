...
const EnrolledCourseCard = async ({enrollment}) => {
    ...
    const filter = {course: enrollment?.course?._id, student: enrollment?.student?._id};
	 ...
	 const totalProgress = (totalCompletedModules / totalModuleCount * 100) || 0
	 const quizzes = report?.quizAssessment?.assessments || []
	 const totalQuizzes = quizzes?.length || 0
	 const quizzesTaken = quizzes?.filter(q => q.attempted) || []
	 const totalCorrect = quizzesTaken?.map((quizze) => {
		return quizze?.options?.filter(q => {
			return q.isCorrect === true && q.isSelected === true
		})
	 }).filter(correctElement => {
		return correctElement.length > 0
	 }).flat()
					...
					<div className=" border-b pb-2 mb-2">
						<div className="flex items-center justify-between">
							...
							<div className="text-md md:text-sm font-medium text-slate-700">
								Completed Modules <Badge variant="success">
